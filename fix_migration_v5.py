...
CREATE UNIQUE INDEX IF NOT EXISTS idx_crm_knowledge_audit_idemp ON public.crm_knowledge_audit_logs(document_id, action, correlation_id) WHERE correlation_id IS NOT NULL;
...
BEGIN
  ...
  IF OLD.knowledge_status IS DISTINCT FROM NEW.knowledge_status THEN
    ...
    IF NEW.knowledge_status = 'APPROVED' THEN
      IF NEW.knowledge_metadata->'provenance'->>'approved_by' != auth.uid()::text THEN RAISE EXCEPTION 'AUTHORIZATION_VIOLATION: approved_by must match auth.uid()'; END IF;
      BEGIN v_ts := (NEW.knowledge_metadata->'provenance'->>'approved_at')::TIMESTAMPTZ; EXCEPTION WHEN OTHERS RAISE EXCEPTION 'VALIDATION_ERROR: approved_at invalid timestamp'; END;
    ...
    IF NEW.knowledge_status = 'ACTIVE' THEN
      IF COALESCE(current_setting('request.jwt.claim.role', true), '') <> 'service_role' 
        ...
        RAISE EXCEPTION 'SECURITY_VIOLATION: Activation requires both service_role and app.verified_webhook_callback flag.'; 
      END IF;
  ...
  IF OLD.ingestion_status IS DISTINCT FROM NEW.ingestion_status THEN
    ...
    IF NEW.ingestion_status IN ('SUCCESS', 'FAILED') THEN
      IF COALESCE(current_setting('app.verified_webhook_callback', true), 'false') <> 'true' THEN 
        RAISE EXCEPTION 'SECURITY_VIOLATION: Ingestion completion requires app.verified_webhook_callback context flag.'; 
      END IF;
    END IF;
    IF NOT (
...
BEGIN
  ...
  -- Verify Trusted Context
  -- For service_role (e.g., Trusted n8n Backend), we inherently trust the requested organization_id.
  -- For non-service_role (e.g., Client Application), enforce membership validation.
  IF COALESCE(current_setting('request.jwt.claim.role', true), '') <> 'service_role' THEN
