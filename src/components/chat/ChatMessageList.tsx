...
export const ChatMessageList = memo(function ChatMessageList({ messages, isTyping, onCommand }: { messages: ChatMessage[], isTyping?: boolean, onCommand?: (cmd: string) => void }) {
  ...
  const openMarkdownPreview = async (url: string, title: string, createdAt?: string) => {
    setMarkdownPreview({ title, url, content: "", loading: true, error: null, createdAt });
    try {
                      ...
                      if (isAttachment) {
                        if (isMarkdownAttachment) {
                          ...
                          return (
                            <button
                              type="button"
                              onClick={() => openMarkdownPreview(href, markdownTitle || 'Markdown attachment', message.created_at)}
          ...
          >
            <div className="flex items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800/80 bg-slate-50/50 dark:bg-slate-900/50 px-6 py-4 backdrop-blur-md">
              <div className="min-w-0">
                <div className="text-[11px] font-medium uppercase tracking-[0.15em] text-slate-500 dark:text-slate-400 mb-1 flex items-center gap-2">
                  <span>{t("chat.markdown.preview")}</span>
