    ...
    <div className="rounded-2xl border border-slate-800/60 bg-slate-900/40 p-6 backdrop-blur-xl shadow-xl shadow-black/20 transition-all hover:bg-slate-900/60 hover:border-slate-700/50">
      <div className="flex items-start justify-between gap-4">
        <div className="flex gap-4">
          ...
          <div>
            <div className="text-xl font-bold bg-gradient-to-r from-white to-slate-400 bg-clip-text text-transparent">{agent.canonical_name}</div>
            <div className="mt-1 text-sm text-slate-400 font-mono">{agent.role_code}</div>
          </div>
        </div>
      ...
      <div className="mt-6 flex flex-col gap-3">
        <div className="flex items-center gap-2 rounded-xl border border-slate-800/60 bg-slate-950/50 p-3 text-sm text-slate-300">
          ...
          <div className="font-mono text-xs font-semibold text-amber-300">{agent.authority_scope}</div>
        </div>
        
        <div className="flex items-center gap-2 text-xs uppercase tracking-widest text-slate-500 mt-2">
          <Building2 className="h-3.5 w-3.5" />
          <span className="truncate">{t("agents.card.department") ?? "Department"}: <span className="font-mono text-slate-400">{agent.department_id.split('-')[0]}...</span></span>
        </div>
      </div>
    </div>
  );
