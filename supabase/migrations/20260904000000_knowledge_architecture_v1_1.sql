...
CREATE UNIQUE INDEX idx_crm_knowledge_idempotency ON public.crm_knowledge_documents(organization_id, idempotency_key) WHERE idempotency_key IS NOT NULL;

-- Remove CASCADE here to avoid silently dropping external dependencies
...
-- PREFLIGHT CHECK: Halt on data anomalies, never auto-delete.
DO $$
BEGIN
  -- Check 1: Orphaned audit rows (no document_id or organization_id)
  IF EXISTS (SELECT 1 FROM public.crm_knowledge_audit_logs WHERE document_id IS NULL OR organization_id IS NULL) THEN
    RAISE EXCEPTION 'MIGRATION_HALTED: Orphaned audit logs detected. Manual quarantine required to prevent data loss.';
  END IF;

  -- Check 2: Tenant mismatch between audit log and referenced document
  IF EXISTS (
    SELECT 1 FROM public.crm_knowledge_audit_logs a
    JOIN public.crm_knowledge_documents d ON a.document_id = d.id
    WHERE a.organization_id != d.organization_id
  ) THEN
    RAISE EXCEPTION 'MIGRATION_HALTED: Tenant mismatch detected between audit logs and documents.';
  END IF;

  -- Check 3: Existing INGESTION audit rows missing correlation_id (would violate upcoming constraint)
  IF EXISTS (
    SELECT 1 FROM public.crm_knowledge_audit_logs
    WHERE action LIKE 'INGESTION_%' AND correlation_id IS NULL
  ) THEN
    RAISE EXCEPTION 'MIGRATION_HALTED: Existing INGESTION audit logs are missing correlation_id. Backfill or quarantine required before applying constraint.';
  END IF;

  -- Check 4: Negative retry_attempt values
  IF EXISTS (SELECT 1 FROM public.crm_knowledge_audit_logs WHERE retry_attempt < 0) THEN
    RAISE EXCEPTION 'MIGRATION_HALTED: Audit logs with negative retry_attempt detected.';
  END IF;
END $$;
...
-- Enforce Mandatory correlation_id for Ingestion Callbacks to guarantee Idempotency
ALTER TABLE public.crm_knowledge_audit_logs DROP CONSTRAINT IF EXISTS chk_audit_correlation;
ALTER TABLE public.crm_knowledge_audit_logs ADD CONSTRAINT chk_audit_correlation CHECK (action NOT LIKE 'INGESTION_%' OR correlation_id IS NOT NULL);
...
CREATE INDEX idx_crm_knowledge_audit_doc ON public.crm_knowledge_audit_logs(document_id);

-- Fixed Idempotency
DROP INDEX IF EXISTS public.idx_crm_knowledge_audit_idemp;
CREATE UNIQUE INDEX idx_crm_knowledge_audit_idemp 
  ON public.crm_knowledge_audit_logs(document_id, action, correlation_id) 
  WHERE correlation_id IS NOT NULL;

-- Secure Audit Logs (Immutability & RLS)
-- REVOKE ALL first, then restore only required grants explicitly.
...
-- UI members read audit via RLS policy; grant SELECT so the policy can be exercised.
...
-- 6. Audit Trigger (Multi-Event Safe)
...
BEGIN
  -- Insert separate event for Knowledge Lifecycle
  IF OLD.knowledge_status IS DISTINCT FROM NEW.knowledge_status THEN
    INSERT INTO public.crm_knowledge_audit_logs (
      organization_id, document_id, actor_id, action, prev_knowledge_status, new_knowledge_status, details, correlation_id, ingestion_run_id, retry_attempt
    ) VALUES (
      NEW.organization_id, NEW.id, auth.uid(), 'KNOWLEDGE_' || NEW.knowledge_status, OLD.knowledge_status, NEW.knowledge_status, NEW.knowledge_metadata, 
  ...
  -- Insert separate event for Ingestion Lifecycle
  IF OLD.ingestion_status IS DISTINCT FROM NEW.ingestion_status THEN
    INSERT INTO public.crm_knowledge_audit_logs (
      organization_id, document_id, actor_id, action, prev_ingestion_status, new_ingestion_status, details, correlation_id, ingestion_run_id, retry_attempt
    ) VALUES (
      NEW.organization_id, NEW.id, auth.uid(), 'INGESTION_' || NEW.ingestion_status, OLD.ingestion_status, NEW.ingestion_status, NEW.knowledge_metadata, 
      NULLIF(current_setting('app.correlation_id', true), ''), 
      NULLIF(current_setting('app.ingestion_run_id', true), '')::UUID, 
      NULLIF(current_setting('app.retry_attempt', true), '')::INT
    );
...
-- 7. RPC Contract (Trusted Backend Only)
-- NOTE: REVOKE/GRANT must come AFTER CREATE OR REPLACE to avoid failure when function doesn't exist.
-- Security assumption: service_role is an internal trusted boundary (n8n/API server).
-- It is not a DB-level tenant isolation guarantee; the API layer must enforce tenant context.
...
-- REVOKE/GRANT after function exists
