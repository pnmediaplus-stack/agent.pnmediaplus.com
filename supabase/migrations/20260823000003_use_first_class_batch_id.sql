...
BEGIN
  ...
  -- Extract explicit batch_id from the latest visual asset
  ...
  ELSE
    -- Fallback for legacy assets before batch_id was introduced
    SELECT array_agg(asset_uri) INTO v_images
    FROM (
      SELECT asset_uri
      FROM pn_content_phase2.assets
      WHERE content_item_id = p_content_item_id AND asset_type = 'visual_asset'
