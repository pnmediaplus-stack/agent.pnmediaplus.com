-- Fix: phase074_create_tenant_integration missed setting current_master_key_id, causing SECRET_PACKAGE_NOT_READY
-- We also run a backfill to fix any recently created credentials.
...
begin
  ...
  update pn_vault.vault_credentials set current_secret_blob_id = v_vault_blob_id, current_master_key_id = v_master_key_id where id = v_credential_id;
...
-- Backfill missing master_key pointers for any credentials created before this fix
